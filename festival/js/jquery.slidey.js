  ...
  $.slidey = function (el, options) {
      ...
      if (base.$el.find("ul").size() > 0) {
        base.$el
          ...
          .each(function () {
            ...
            slide.title =
              $this.find("p.title").size() > 0
                ? $this.find("p.title").first().html()
                : "";
            slide.description =
              $this.find("p.description").size() > 0
                ? $this.find("p.description").first().html()
                : "";
    ...
    base.constructLayout = function () {
      ...
      if (base.options.showList) {
        domdocument +=
          "<div class='slidey-row row'><div class='slidey-image col-md-8'><div class='slidey-overlay'>";
      } else {
        domdocument +=
          "<div class='slidey-row row'><div class='slidey-image col-md-12'><div class='slidey-overlay'>";
      }
      domdocument +=
        "<p class='slidey-overlay-title'></p><p class='slidey-overlay-description'></p>";
      domdocument += "<span class='slidey-progress'></span>";
      domdocument +=
        "</div></div><div class='slidey-list col-md-4'></div></div>";
      ...
      base.layout.overlay.$title = base.$el
        .find(".slidey-overlay-title")
        .first();
      base.layout.overlay.$description = base.$el
        .find(".slidey-overlay-description")
        .first();
      base.layout.overlay.$progress = base.$el.find(".slidey-progress").first();

      // create controls
      var controls =
        "<div class='slidey-controls slidey-controls-previous'><i class='fa fa-chevron-left'></i></div>";
      controls +=
        "<div class='slidey-controls slidey-controls-next'><i class='fa fa-chevron-right'></i></div>";
      ...
      for (var slideIndex = 0; slideIndex < base.slides.length; slideIndex++) {
        ...
        elements +=
          "<div class='slidey-list-thumbnail' style='background-image: url(\"" +
          base.slides[slideIndex].image +
          "\")'>";
        elements +=
          "</td><td class='slidey-list-title'>" +
          base.slides[slideIndex].title +
          "</td>";
        elements += "</tr><tr>";
        elements +=
          "<td class='slidey-list-description'>" +
          base.slides[slideIndex].description +
          "</td>";
        elements += "</tr></table>";
        $li.append(elements);
        $list.append($li);
      ...
      var oneSlideListItemHeight =
        parseInt(sliderHeight) / base.options.listCount;

      // set list item thumbnail dimensions
      var thumbWidth = oneSlideListItemHeight - 9;
      base.layout.$list
        .find(".slidey-list-thumbnail")
      ...
      base.layout.$list.find("li").each(function () {
        ...
        $description.width(
          $this.innerWidth() -
            $(".slidey-list-thumbnail-container", $this).width()
        );
    ...
    base.stopTimer = function () {
      ...
      if (base.layout.overlay.progressWidth === -1) {
        base.layout.overlay.progressWidth =
          base.layout.overlay.$progress.originalWidth();
      }
    };
    ...
    base.gotoSlide = function (slideIndex) {
      ...
      if (base.options.showList) {
        base.layout.$list
          .find("li")
          .eq(slideIndex)
          .scrollToView(".slidey-list", base.options.listCount);
      }
      base.layout.$overlay.fadeOut("fast", function () {
        ...
        base.layout.$image.css(
          "background-image",
          "url(" + base.slides[slideIndex].image + ")"
        );
        base.layout.overlay.$title
          .empty()
          .append(base.slides[slideIndex].title);
        base.layout.overlay.$description
          .empty()
          .append(base.slides[slideIndex].description);
    ...
    base.prepareNodes = function () {
      if (base.options.showNodes) {
        if (
          base.options.nodeContainer !== "" &&
          $(base.options.nodeContainer).size() > 0
        ) {
          base.layout.$nodeContainer = $(base.options.nodeContainer);
          var $nodebase = $("<div class='slidey-node-container'></ul>");
          for (
            var slideIndex = 0;
            slideIndex < base.slides.length;
            slideIndex++
          ) {
            var node =
              "<div class='slidey-node' style='background-image: url(\"" +
              base.slides[slideIndex].image +
              "\");'></div>";
          ...
          $(".slidey-node", base.layout.$nodeContainer).on(
            "click",
            function () {
              base.gotoSlide(
                base.layout.$nodeContainer.find(".slidey-node").index($(this))
              );
            }
          );
        }
    ...
    if ($parent) {
      if (itemIndex >= listCount) {
        var $itemToHide = $this
          .parent()
          .find("li")
          .eq(itemIndex - listCount);
        $parent.stop().animate(
          {
            scrollTop:
              $itemToHide.position().top + $itemToHide.originalHeight() + 2,
          },
          "300",
          "swing"
        );
